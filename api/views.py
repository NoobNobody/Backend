...
class FiltrateAndSearchJobOffersByCategory(APIView):
    def get(self, request, category_id, *args, **kwargs):
        paginator = StandardResultsSetPagination()
        search_query = request.query_params.get('search', None)
        offers_query = JobOffers.objects.filter(Category_id=category_id).select_related('Website', 'Category')
...
class FiltrateAndSearchAllJobOffers(APIView):
    def get(self, request, *args, **kwargs):
        paginator = StandardResultsSetPagination()
        search_query = request.query_params.get('search', None)
        ...
        offers_query = JobOffers.objects.all().select_related('Website', 'Category')
